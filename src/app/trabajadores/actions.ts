...
const workerSchema = z.object({
  ...
  cargoId: z.string().trim().optional(),
  proyectoId: z.string().trim().optional(),
  trabajoPrevioMandante: z.string().trim().optional(),
...
export async function createWorkerAction(formData: FormData) {
  ...
  const parsed = workerSchema.safeParse({
    ...
    trabajoPrevioMandante: String(formData.get("trabajoPrevioMandante") ?? ""),
  ...
  const worker = await db.staffMember.create({
    data: {
      ...
      trabajoPrevioMandante: payload.trabajoPrevioMandante === "on",
